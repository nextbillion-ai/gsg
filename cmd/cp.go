...
func download(src, dst *system.FileObject, forceChecksum, isRec bool, wg *sync.WaitGroup) {
	...
	case system.FileType_Directory:
		if isRec {
			...
			for _, obj := range objs {
				...
				pool.Add(func() {
					defer wg.Done()
					if err = src.System.Download(src.Bucket, srcPath, dstPath, forceChecksum, system.RunContext{Bars: bars, Pool: pool, ChunkSize: chunkSize, DirectIO: directIO}); err != nil {
						common.Exit()
					}
				})
			}
	...
	case system.FileType_Object:
		...
		if err = src.System.Download(src.Bucket, src.Prefix, dstPrefix, forceChecksum, system.RunContext{Bars: bars, Pool: pool, ChunkSize: chunkSize, DirectIO: directIO}); err != nil {
			common.Exit()
		}
	case system.FileType_Invalid:
...
func interCloudCopy(src, dst *system.FileObject, forceChecksum, isRec bool, wg *sync.WaitGroup) {
		...
		if isRec {
			...
			for _, obj := range objs {
				...
				pool.Add(func() {
					var err error
					defer wg.Done()
					if err = src.System.Download(src.Bucket, srcPath, interPath, forceChecksum, system.RunContext{Bars: bars, Pool: pool, ChunkSize: chunkSize, DirectIO: directIO}); err != nil {
						common.Exit()
					}
	...
	case system.FileType_Object:
		...
		if err = src.System.Download(src.Bucket, src.Prefix, interPath, forceChecksum, system.RunContext{Bars: bars, Pool: pool, ChunkSize: chunkSize, DirectIO: directIO}); err != nil {
			common.Exit()
		}
		interFile := system.ParseFileObject(interPath)
		if interFile.FileType() != system.FileType_Object {
