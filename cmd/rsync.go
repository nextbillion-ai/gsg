...
func downsync(src, dst *system.FileObject, isRec, isDel, forceChecksum bool) {
	...
	for _, fo := range copyList {
		if e := common.DoWithRetrySimple(func() error {
			return fo.System.Download(fo.Bucket, fo.Prefix, common.JoinPath(dst.Prefix, fo.Attributes.RelativePath), forceChecksum, system.RunContext{Pool: pool, Bars: bars, ChunkSize: chunkSize, DirectIO: directIO})
		}); e != nil {
			common.Exit()
		}
	}
